...
const Dropzone = props => {
  ...
  function nameEdit(v, url) {
    ...
    if (namefetch[1]) {
      name = namefetch[0].slice(0, 1).toUpperCase() + "" + namefetch[1].slice(0, 1).toUpperCase();
    }
    else {
      name = fetch[0].slice(0, 1).toUpperCase();
    }
    num = fetch[1];
  ...
  function selectImage(e) {
    ...
    if (fetch.length > 2) {
      name = fetch[0].slice(0, 1).toUpperCase() + "" + fetch[1].slice(0, 1).toUpperCase();
      num = fetch[2];
    }
    else {
      name = fetch[0].slice(0, 1).toUpperCase();
